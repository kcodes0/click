export const GAME_TENTS_JS = String.raw`(function () {
  ...
  var treeSet = {};
  gridData.trees.forEach(function (t) { treeSet[t] = true; });
  var N = W * H;

  // 0 = empty, 1 = tent, 2 = grass (marked empty)
  ...
  var cells = document.querySelectorAll(".tents-cell");
  cells.forEach(function (el) {
    el.addEventListener("click", function () {
      if (solved) return;
      var idx = parseInt(el.getAttribute("data-idx"));
      if (treeSet[idx]) return;
  ...
  function render(idx) {
    ...
    if (!el) return;
    if (treeSet[idx]) return;
    el.className = "tents-cell" +
      (state[idx] === 1 ? " tents-cell--tent" : "") +
      (state[idx] === 2 ? " tents-cell--grass" : "");
    el.textContent = state[idx] === 1 ? "\u26fa" : (state[idx] === 2 ? "\u00d7" : "");
  }

  function checkSolved() {
    var answer = [];
    for (var i = 0; i < N; i++) if (state[i] === 1) answer.push(i);
    if (answer.length !== gridData.trees.length) return;
    ...
    fetch("/api/solves", {
      ...
      if (d.ok) {
        ...
        document.querySelector(".tents-table").classList.add("solved");
